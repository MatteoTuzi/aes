...
/* -------------------------------------------------------------------------
   ...
   - IRQ1 (Button): Inverts the current LED pattern with debounce
   ------------------------------------------------------------------------- */
...
void myISR (void) {
    ...
    // Handle Button interrupt (IRQ1)
    if (pending_interrupts & IRQ_BTN_MASK) {
        // Software debounce: wait 50ms to filter mechanical bounce
        usleep(50000);

        // Read current LED value and invert it (4 LSB bits)
        led_value = Xil_In32(GPIO_LED_BASE + GPIO_DATA) & 0xF;
        led_value = (~led_value) & 0xF;
        
        // Write inverted value to LEDs
        Xil_Out32(GPIO_LED_BASE + GPIO_DATA, led_value);
